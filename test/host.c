...
int main(int argc,char **argv){
 if(argc==2&&!strcmp(argv[1],"phrases")){for(int i=0;i<1440;i++){ProsePhrase p=prose_phrase(i/60,i%60);printf("%s|",p.exact);for(int j=0;j<p.count;j++)printf("%s%s",j?" ":"",prose_run_text(p.ids[j]));puts("");}return 0;}
