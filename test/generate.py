...
LAYOUTS=[
 {'width':144,'height':168,'sizes':(18,27,38,12),'top':(22,47,82,106),'exact_top':(38,65,111),'footer_top':151,'footer_step':8,'rule_length':12,'platforms':['BASALT','DIORITE','FLINT']},
 {'width':180,'height':180,'sizes':(20,31,43,12),'top':(23,50,89,116),'exact_top':(42,70,125),'footer_top':159,'footer_step':8,'rule_length':13,'platforms':['CHALK']},
 {'width':200,'height':228,'sizes':(25,38,53,16),'top':(32,65,115,150),'exact_top':(55,90,158),'footer_top':204,'footer_step':10,'rule_length':17,'platforms':['EMERY']},
 {'width':260,'height':260,'sizes':(29,44,60,18),'top':(38,79,131,168),'exact_top':(67,109,190),'footer_top':233,'footer_step':12,'rule_length':20,'platforms':['GABBRO']},
]
...
def build():
 ...
 header=['// Generated font bitmap data retains the SIL OFL. See resources/fonts/ for credits and licenses.\n#pragma once\n#include <stdint.h>\n#include <stddef.h>\ntypedef struct {uint32_t offset;uint16_t w,h;} ProseSprite;\ntypedef struct {int width,height;int top[4],exact_top[3];int footer_top,footer_step,footer_height,rule_length;const ProseSprite *sprites;const uint8_t *data;} ProseLayout;\n']
 metadata=[]
 for index,layout in enumerate(LAYOUTS):
  sizes=layout['sizes'];data=[];sprites=[]
  for id,text in enumerate(WORDS+list('0123456789:')):
   italic=id==0 or 7<=id<=9
   size=sizes[3] if id>=22 else sizes[0] if italic else sizes[1] if id<=6 else sizes[2]
   font=load_font(size,italic)
   if id>=22:
    font=ImageFont.truetype(str(ROOT/'resources/fonts/JetBrainsMono[wght].ttf'),size);font.set_variation_by_axes([500])
   box=font.getbbox(text)
   # Shared vertical metrics keep each semantic role on the same baseline.
   group='0123456789:' if id>=22 else 'it is past till oclock' if italic else ' '.join(WORDS[1:7] if id<=6 else WORDS[10:22])
  ...
  fields=[str(layout['width']),str(layout['height']),'{'+','.join(map(str,layout['top']))+'}','{'+','.join(map(str,layout['exact_top']))+'}',str(layout['footer_top']),str(layout['footer_step']),str(sprites[22][2]),str(layout['rule_length']),'prose_sprites_'+str(index),'prose_data_'+str(index)]
