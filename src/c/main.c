...
static void draw(Layer*layer,GContext*ctx){
 time_t begin;uint16_t begin_ms;time_ms(&begin,&begin_ms);time_t now=time(NULL);struct tm t=*localtime(&now);GRect b=layer_get_bounds(layer);
 ...
 Destination d={frame};prose_render(b.size.w,b.size.h,t.tm_hour,t.tm_min,PBL_IF_COLOR_ELSE(false,true),put_pixel,&d);graphics_release_frame_buffer(ctx,frame);
